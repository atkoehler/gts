...
#
def test(locations, test_obj, source, file_paths, env, alterations, n):
    OK = 0
    ...
    if len(file_paths) > 0: 
        ...
        for input in file_paths:
            ...
            if status[testing_file] and len(open(outf).read()) > 0: 
                ...
                testfile_output_string = open(outf).read()
            else:
                testfile_output = ""
            ...
            if status[env["solution_name"]] and len(open(outf).read()) > 0: 
                solution_output = open(outf).readlines()
                solution_output_string = open(outf).read()
            else:
                solution_output = ""
            ...
            if len(diffoutput) > 0:
                ...
                if len(testfile_output_string.strip()) == 0:
                    ...
                    s += "some tests. Check the specification and make sure "
...
def dicttest(locations, test_obj, source, file_paths, 
    ...
    if len(file_paths) > 0: 
            ...
            for line in testfile_output:
                if len(line.strip()) != 0:
                    if line.decode('utf-8').lower().find(keyval.lower()) != -1:
                        ...
                        space_aft = line.decode('utf-8').lower().find(" ", key_loc+len(keyval))
                        if len(testfile_output_string) > 0:
                            testfile_output_string += "\n"
                        if space_aft != -1:
        ...
        for line in testfile_output:
            if len(line.strip()) != 0:
                    if line.lower().find(keyval.lower()) != -1:
                        testfile_output_string = line.strip().split()[-1]
                        tf_output.append(line.strip().lower().split()[-1]+'\n')
        ...
        for line in solution_output:
            if len(line.strip()) != 0:
                if line.lower().find(keyval.lower()) != -1:
                    solution_output_string = line.strip().split()[-1]
                    sol_output.append(line.strip().lower().split()[-1]+'\n')
...
# @param env the environment variables dict from JSON config of test suite
# @param alterations dict of lists of alterations to source before testing it
# @param keys list of keys to utilize when comparing
# @param n the number of failed cases to output
#
# @return 0 if test completed successfully, otherwise -1
#
def dicttest_nkeys(locations, test_obj, source, file_paths, 
             env, alterations, keys, n):
    OK = 0
    from modules.compilation import compile_single
    import modules.altersource as altersource
    import difflib
            ...
            for keyval in keys:
                with open(input, 'r') as inp_file:            
                    try:
                        ...
                        t = Task(env["timeout"])
                        with open(outf, 'w+') as out_file:
                            t.check_call([exe_path], stdout=out_file, 
                                         stderr=FNULL, 
                                         stdin=inp_file)
                        status[testing_file] = True                
                    except SystemError as e:
                        status[testing_file] = False
                        # create a notice about abort throw and put in message
                        m = "Your program's execution was aborted."
                        notifications["abort"] = m
                        ...
                        if e[0][0] == -11:
                            cl = ["return value", "values of each parameter"]
                            s = "A segmentation fault occurred when running "
                            s += "your "
                            s += "program."
                            suggestions.append(s)
                        # termination of thread due to timeout
                            ...
                            cl = ["infinite loops (check your conditions)", 
                                  "attempting to get input when none exists"]
                            s = "Your program failed to execute "
                            s += "to completion in the time allotted to it. "
                            s += "This may have been caused by several things "
                            s += "including:\n"
                            s += markup_create_indent(markup_create_unlist(cl),
                                                      1)
                            suggestions.append(s)
                ...
                tf_output = []
                testfile_output_string = ""
                expand_all_tabs(testfile_output, 3)
                for line in testfile_output:
                    if len(line.strip()) != 0:
                        if line.decode('utf-8').lower().find(keyval.lower()) != -1:
                            key_loc = line.decode('utf-8').lower().find(keyval.lower())
                ...
                sol_output = []
                solution_output_string = ""
                expand_all_tabs(solution_output, 3)
                for line in solution_output:
                    if len(line.strip()) != 0:
                        if line.decode('utf-8').lower().find(keyval.lower()) != -1:
                            key_loc = line.decode('utf-8').lower().find(keyval.lower())
                ...
                if len(diffoutput) > 0:
                    ...
                    rec_hdr = markup_create_indent(rec_hdr, 2)
                    if len(testfile_output_string) > 0:
                        rec_contents = markup_create_codeblock("\n".join(testfile_output))
                        rec_contents = markup_create_indent(rec_contents, 3)
                    else:
                        rec_contents = "\n"
                    ...
                    sol_hdr = markup_create_indent(sol_hdr, 2)
                    if len(solution_output_string) > 0:
                        sol_contents = markup_create_codeblock("\n".join(solution_output))
                        sol_contents = markup_create_indent(sol_contents, 3)
                    else:
                        if "13_1000000.test" == input[input.rfind("/")+1:]:
                            print "Failed to find output using key,", keyval
                        sol_contents = "No output is expected for this"
                        sol_contents += ".\n"
    ...
    # make suggestions and messages unique
    messages = unique_list(messages)
    suggestions = unique_list(suggestions)
...
# @return 0 if test completed successfully, otherwise -1
#
def size_nkeys(locations, test_obj, source, file_paths, 
             env, alterations, keys, n):
    OK = 0
    from modules.compilation import compile_single
    import modules.altersource as altersource
    ...
    if len(file_paths) > 0: 
        ...
        for input in file_paths:
            ...
            for keyval in keys:
                ...
                if len(solution_output_string) != len(testfile_output_string):
                    diffoutput = "You are not properly formatting your "
                    diffoutput += "estimated probabilites to the proper amount "
                    diffoutput += "of digits after the decimal."
                else:
                    diffoutput = ""
            ...
            if interim_score == len(keys):
                test_obj.score += float(test_obj.max_score) / len(file_paths)
    #else:
        #TODO: else statement for NO files
